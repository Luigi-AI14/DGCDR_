...
def call_llm(host, model, system_prompt, user_prompt, num_ctx, think, timeout):
    payload = {
        'model': model,
        'messages': [
    ...
    response = requests.post(host.rstrip('/') + '/api/chat', json=payload, timeout=timeout)
    ...
    try:
        ...
        raise SystemExit('model returned unparseable JSON ({}):\n{}'.format(exc, content[:2000]))
    return parsed, body
...
def aggregate(summaries, args):
    ...
    totals['rescued_rate'] = (round(totals['llm_rescued'] / float(totals['n_candidates']), 3)
                              if totals['n_candidates'] else None)
    ...
    if probes:
        ...
        totals['probe'] = {
            ...
            'recall_on_known_positives': round(n_rec / float(n_probe), 3) if n_probe else None,
        }
    return totals
...
def main():
    ...
    summaries = [s for s in (audit_user(p, args, rng) for p in paths) if s]
    ...
    if len(summaries) > 1:
        ...
        if 'probe' in totals:
            print('probe recall on known positives: {}'.format(
                totals['probe']['recall_on_known_positives']))
